...
module.exports = (client) => {
  ...
  for (const eventFolder of eventFolders) {
    ...
    client.on(eventName, async (...args) => { // Привязываем обработчик к событию `eventName`
      for (const eventFile of eventFiles) {
        const eventFunction = require(eventFile); // Импортируем функцию обработчика из каждого файла в папке события
        await eventFunction(client, ...args); // Вызываем функцию обработчика с клиентом и аргументами события
      }
    });
  }
};
